...
```{r}
library(faraway)
round(cor(data[-181]), 2)[1,1:20] # Correlation of data; as shown in the output, the correlation is large
```
```{r}
...
```
```{r}
coef(linear_model)[1:10] ## coefficients of predictors
...
```{r}
round(cor(new_data[-181]), 2)[1,1:30]
```
...
```
```{r}
coef(linear_model_out)[1:10] ## coefficients of predictors
...
sum(mod_cd > 4 / length(mod_cd))
boxcox_bic = lm(((Annual_MIR ^ 0.2 - 1)  / 0.2) ~ ., data = bic_data,subset = mod_cd < 4 / length(mod_cd) )
```
